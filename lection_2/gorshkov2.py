...
def remove_element(element, array):  # declarate the function
    return [
        number for number in array if number != element
    ]  # describing the formula excepting search element
...
formatted_strings = [
    add_space(s) for s in strings
]  # creating reformated string + spaces
...
)  # print reworked array according function logic
print(
...
def find_max_frequency(input_string):  # declarate function
    ...
    max_freq_chars = [
        [char, count] for char, count in freq.items() if count == max_freq
    ]
